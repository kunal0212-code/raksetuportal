...
import { submitEmergencyRequest } from '../data/storage';
...
const EmergencyRequest = () => {
    ...
    const validate = () => {
        const err = {};
        if (!formData.patientName.trim()) err.patientName = 'Patient name is required';
        if (!formData.bloodGroup) err.bloodGroup = 'Select blood group';
        if (!formData.unitsNeeded || Number(formData.unitsNeeded) < 1) err.unitsNeeded = 'At least 1 unit required';
        if (!formData.hospital.trim()) err.hospital = 'Hospital name is required';
        if (!formData.city.trim()) err.city = 'City is required';
        if (!formData.contactName.trim()) err.contactName = 'Contact name is required';
        if (!formData.contactPhone.trim()) err.contactPhone = 'Phone number is required';
        else if (!/^[6-9]\d{9}$/.test(formData.contactPhone.replace(/\s/g, '')))
    ...
    const handleSubmit = async (e) => {
        ...
        const result = submitEmergencyRequest(formData);
        setRequest(result);
